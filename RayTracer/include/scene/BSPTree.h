...
class BSPNode {
        ...
        if (pChild[0] == NULL) {
            ...
            if (raypos_coord < subdiv_plane_coord) {
                ...
                if (dir > 0) {
                    ...
                    //dir == 0
                    //If this happens, it'll only intersect objects that are in both sides, so this can be folded into
                the above cases
  ...
  private:
    double tValueToSubdivisionPlane(const Vec3d &ptInPlane,
                                    const ray &r) const // ray's "t" value at intersection with the subdivision plane
    {
...
class BSPTree {
  ...
  public:
    ...
    void initialize(std::vector<Geometry *> *pObjectList, unsigned short maxTreeDepth, unsigned long maxChildrenPerNode,
