...
class DirectionalLight : public Light {
  public:
    DirectionalLight(Scene *scene, const Vec3d &orien, const Vec3d &color) : Light(scene, color), orientation(orien) {
...
class PointLight : public Light {
  public:
    PointLight(Scene *scene, const Vec3d &pos, const Vec3d &color, float constantAttenuationTerm,
               float linearAttenuationTerm, float quadraticAttenuationTerm)
        : Light(scene, color), position(pos), constantTerm(constantAttenuationTerm), linearTerm(linearAttenuationTerm),
