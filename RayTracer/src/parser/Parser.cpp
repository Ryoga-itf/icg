...
void Parser::parseCamera(Scene *scene) {
    ...
    for (;;) {
        ...
        case QUATERNIAN:
            ...
            scene->getCamera().setLook(quaternian[0], quaternian[1], quaternian[2], quaternian[3]);
            break;
...
void Parser::parseTranslate(Scene *scene, TransformNode *transform, const Material &mat) {
    ...
    parseTransformableElement(scene, transform->createChild(Mat4d::createTranslation(x, y, z)), mat);
...
void Parser::parseRotate(Scene *scene, TransformNode *transform, const Material &mat) {
    ...
    // Parse child geometry
    parseTransformableElement(scene, transform->createChild(Mat4d::createRotation(w, x, y, z)), mat);
...
void Parser::parseScale(Scene *scene, TransformNode *transform, const Material &mat) {
    ...
    // Parse child geometry
    parseTransformableElement(scene, transform->createChild(Mat4d::createScale(x, y, z)), mat);
...
void Parser::parseTransform(Scene *scene, TransformNode *transform, const Material &mat) {
    ...
    parseTransformableElement(scene, transform->createChild(Mat4d(row1, row2, row3, row4)), mat);
...
void Parser::parseCone(Scene *scene, TransformNode *transform, const Material &mat) {
    ...
    for (;;) {
        ...
        case RBRACE:
            _tokenizer.Read(RBRACE);
            cone = new Cone(scene, newMat ? newMat : new Material(mat), height, bottomRadius, topRadius, capped);
        ...
        case RBRACE: {
            ...
            for (list<Vec3d>::const_iterator vitr = faces.begin(); vitr != faces.end(); vitr++) {
                if (!tmesh->addFace((int)(*vitr)[0], (int)(*vitr)[1], (int)(*vitr)[2])) {
                    ostringstream oss;
                    oss << "Bad face in trimesh: (" << (*vitr)[0] << ", " << (*vitr)[1] << ", " << (*vitr)[2] << ")";
...
PointLight *Parser::parsePointLight(Scene *scene) {
    ...
    for (;;) {
        ...
        case RBRACE:
            ...
            _tokenizer.Read(RBRACE);
            return new PointLight(scene, position, color, constantAttenuationCoefficient, linearAttenuationCoefficient,
                                  quadraticAttenuationCoefficient);

        default:
            throw SyntaxErrorException("expecting 'position' or 'color' attribute, or 'constant_attenuation_coeff', "
                                       "'linear_attenuation_coeff', or 'quadratic_attenuation_coeff'",
                                       _tokenizer);
        }
    }
}
...
DirectionalLight *Parser::parseDirectionalLight(Scene *scene) {
    ...
    for (;;) {
        ...
        default:
            throw SyntaxErrorException("expecting 'position' or 'color' attribute", _tokenizer);
        }
    }
}
