...
void save(const char *filename, const unsigned char *imageBuffer, int width, int height, const char *type,
          int quality) {
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            for (int rgb = 0; rgb < 3; rgb++) {
                shuffled[rgb * width * height + y * width + x] =
                    imageBuffer[(height - 1 - y) * 3 * width + 3 * x + rgb];
            }
        }
    }
