...
DebuggingView::DebuggingView(int x, int y, int w, int h, char *label)
    : Fl_Gl_Window(x, y, w, h, label), raytracer(0), m_shadingMode(NORMAL), m_quality(HIGH_QUALITY),
      m_lightingMode(DEFAULT_LIGHTING), m_useSceneMaterials(true), m_useSceneTextures(true), m_dirty(true),
      m_showGeometry(true), m_showLights(true), m_showCamera(true), m_showAxes(true), m_showNormals(true),
      m_showVisibilityRays(true), m_showReflectionRays(true), m_showRefractionRays(true), m_showShadowRays(false) {
...
void DebuggingView::lightScene() {
    ...
    // Reset all the lights
    {
        ...
        for (std::vector<GLenum>::const_iterator glLightsItr = lights.begin(); glLightsItr != lights.end();
             ++glLightsItr) {
    ...
    if (m_lightingMode == SCENE_LIGHTING) {
        ...
        for (std::vector<Light *>::const_iterator l = raytracer->getScene().beginLights();
            ...
            if (glLightsItr == lights.end())
                std::cerr << "Not displaying all lights in debugging mode; GL can only handle 8." << std::endl;
...
void DebuggingView::drawScene() {
    ...
    raytracer->getScene().glDraw(divisions, m_useSceneMaterials, m_useSceneTextures);
}

void DebuggingView::drawRays() {
    glDisable(GL_LIGHTING);
    // Now draw all the rays
    for (std::vector<std::pair<ray, isect>>::const_iterator rayItr = raytracer->getScene().intersectCache.begin();
...
void DebuggingView::drawCamera() {
    ...
    if (m_dirty && raytracer->isReady()) {
        ...
        gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGB, bufWidth, bufHeight, GL_RGB, GL_UNSIGNED_BYTE, buffer);
    }
    ...
    glPushMatrix();
    const Camera &sceneCamera = raytracer->getScene().getCamera();
    glTranslated((sceneCamera.getEye())[0], (sceneCamera.getEye())[1], (sceneCamera.getEye())[2]);

    glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
    // Now need to draw the camera.
    glBegin(GL_LINES);
    glVertex3d(0, 0, 0);
    glVertex3dv((sceneCamera.getLook() + 0.5 * sceneCamera.getU() + 0.5 * sceneCamera.getV()).getPointer());
    glVertex3d(0, 0, 0);
    glVertex3dv((sceneCamera.getLook() + 0.5 * sceneCamera.getU() - 0.5 * sceneCamera.getV()).getPointer());
    glVertex3d(0, 0, 0);
    glVertex3dv((sceneCamera.getLook() - 0.5 * sceneCamera.getU() + 0.5 * sceneCamera.getV()).getPointer());
    ...
    glTranslated((sceneCamera.getLook())[0], (sceneCamera.getLook())[1], (sceneCamera.getLook())[2]);
