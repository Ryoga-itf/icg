...
void ModelerApplication::Init(ModelerViewCreator_f createView, const ModelerControl controls[], unsigned numControls) {
    int i;
    ...
    for (i = 0; i < m_numControls; i++) {
        m_ui->addControl(controls[i].m_name, controls[i].m_minimum, controls[i].m_maximum, controls[i].m_stepsize,
                         controls[i].m_value);
    }
...
double ModelerApplication::GetControlValue(int controlNumber) { return m_ui->controlValue(controlNumber); }

void ModelerApplication::SetControlValue(int controlNumber, double value) { m_ui->controlValue(controlNumber, value); }
...
void ModelerApplication::setAnimating(bool a) { m_animating = a; }
