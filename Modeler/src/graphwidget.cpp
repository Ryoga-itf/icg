...
static const Fl_Color flcvColors[CURVE_COLOR_COUNT] = {FL_BLACK, FL_RED, FL_GREEN, FL_BLUE, FL_MAGENTA, FL_CYAN};

const static float ks_fViewportMargin = 0.01f;

GraphWidget::GraphWidget(int x, int y, int w, int h, const char *label)
    : Fl_Gl_Window(x, y, w, h, label), m_bHasCtrlPtSelection(false), m_bHasZoomSelection(false), m_ivvCurrCtrlPts(),
      m_ppceCurveEvaluators(NULL), m_pcrvvCurves(), m_ivCurveTypes(), m_iCurrCurve(-1), m_ivActiveCurves(),
      m_fEndTime(20.0f), m_fCurrTime(0.0f), m_rectCurrViewport(0.0f - ks_fViewportMargin, 1.0f + ks_fViewportMargin,
                                                               0.0f - ks_fViewportMargin, 1.0f + ks_fViewportMargin),
      m_bPanning(false), m_bHasEvent(false), m_bLButtonDown(false), m_bRButtonDown(false), m_flcCurrCurve(FL_BLACK) {
...
void GraphWidget::curveType(int iCurve, int iCurveType) {
    if (iCurve >= 0 && iCurve < m_pcrvvCurves.size() && iCurveType >= 0 && iCurveType < CURVE_TYPE_COUNT) {
...
void GraphWidget::currCurveType(int iCurveType) {
    if (m_iCurrCurve >= 0 && iCurveType >= 0 && iCurveType < CURVE_TYPE_COUNT) {
...
void GraphWidget::draw() {
    ...
    if (iLongMarkCountX > 0 && iWindowWidth > 0) {
        ...
        do {
            iMarkX =
                2 * (int)(((double)iStartX * dLongMarkLengthX - leftTime()) / dRangeX * (double)iWindowWidth + 0.5) -
                w();
            x = (double)iMarkX / w();

            int iStartY = (int)ceil(leftTime() / dLongMarkLengthY);
            do {
                iMarkY = 2 * (int)(((double)iStartY * dLongMarkLengthY - leftTime()) / dRangeY * (double)iWindowHeight +
                                   0.5) -
                         h();
                y = (double)iMarkY / h();
...
float GraphWidget::leftTime() const { return m_fEndTime * m_rectCurrViewport.left(); }

float GraphWidget::rightTime() const { return m_fEndTime * m_rectCurrViewport.right(); }

float GraphWidget::currTime() const { return m_fCurrTime; }
...
float GraphWidget::endTime() const { return m_fEndTime; }
...
void GraphWidget::selectAddCtrlPt(const int iMouseX, const int iMouseY) {
    if (m_iCurrCurve >= 0) {
        ...
        if (fabs(ptCtrlPtInWindowCoord.x - ptMouse.x) * 2 <= PICK_WINDOW_SIZE &&
            ...
            if (std::find(m_ivvCurrCtrlPts[m_iCurrCurve].begin(), m_ivvCurrCtrlPts[m_iCurrCurve].end(),
        ...
        for (int i = 0; i < m_ivActiveCurves.size(); ++i) {
            ...
            if (fabs(ptCtrlPtInWindowCoord.x - ptMouse.x) * 2 <= PICK_WINDOW_SIZE &&
                ...
                if (std::find(m_ivvCurrCtrlPts[iCurve].begin(), m_ivvCurrCtrlPts[iCurve].end(), iClosestCtrlPt) !=
    ...
    if (m_ivActiveCurves.size() > 0) {
        ...
        for (int i = 0; i < m_ivActiveCurves.size(); ++i) {
            ...
            m_pcrvvCurves[iCurve]->moveControlPoints(m_ivvCurrCtrlPts[iCurve], ptOffset,
                                                     m_cdvCurveDomains[iCurve].minimum(),
                                                     m_cdvCurveDomains[iCurve].maximum());
        }
...
void GraphWidget::doSelection(const int iMouseX, const int iMouseY) { m_rectSelectionRect.topRight(iMouseX, iMouseY); }
...
void GraphWidget::doZoomSelection(const int iMouseX, const int iMouseY) { doSelection(iMouseX, iMouseY); }
...
void GraphWidget::endZoomSelection(const int iMouseX, const int iMouseY) {
    ...
    if (m_rectSelectionRect.width() > 0.0f && m_rectSelectionRect.height() > 0.0f) {
        ...
        m_rectCurrViewport.left(rectOldViewport.width() * m_rectSelectionRect.left() / (float)w() +
                                rectOldViewport.left());
        m_rectCurrViewport.right(rectOldViewport.width() * m_rectSelectionRect.right() / (float)w() +
                                 rectOldViewport.left());
        m_rectCurrViewport.bottom(rectOldViewport.height() * ((float)h() - m_rectSelectionRect.top()) / (float)h() +
                                  rectOldViewport.bottom());
        m_rectCurrViewport.top(rectOldViewport.height() * ((float)h() - m_rectSelectionRect.bottom()) / (float)h() +
                               rectOldViewport.bottom());
    }
}
...
void GraphWidget::doCtrlPtSelection(const int iMouseX, const int iMouseY) { doSelection(iMouseX, iMouseY); }
...
void GraphWidget::wrapCurve(int iCurve, bool bWrap) { m_pcrvvCurves[iCurve]->wrap(bWrap); }
...
Curve *GraphWidget::curve(int iCurve) const { return m_pcrvvCurves[iCurve]; }
...
void GraphWidget::drawCurve(int iCurve, int iColor) const {
    ...
    glPushMatrix();
    glLoadIdentity();
    gluOrtho2D(m_fEndTime * m_rectCurrViewport.left(), m_fEndTime * m_rectCurrViewport.right(),
...
Point GraphWidget::curveToWindow(int iCurve, const Point &ptCurve) const {
    ...
    ptWindow.x =
        (ptCurve.x - m_rectCurrViewport.left() * m_fEndTime) / (m_rectCurrViewport.width() * m_fEndTime) * (float)w();

    float fNormalizedCurveY = (ptCurve.y - m_cdvCurveDomains[iCurve].minimum()) / m_cdvCurveDomains[iCurve].mag();
    ptWindow.y =
        (float)h() - (fNormalizedCurveY - m_rectCurrViewport.bottom()) / m_rectCurrViewport.height() * (float)h();
