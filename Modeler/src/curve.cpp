...
Curve::Curve() : m_pceEvaluator(NULL), m_bWrap(false), m_bDirty(true), m_fMaxX(1.0f) { init(); }

Curve::Curve(const float fMaxX, const Point &point)
    : m_pceEvaluator(NULL), m_bWrap(false), m_bDirty(true), m_fMaxX(fMaxX) {
...
Curve::Curve(std::istream &isInputStream) { fromStream(isInputStream); }
...
bool Curve::wrap() const { return m_bWrap; }
...
int Curve::controlPointCount() const { return m_ptvCtrlPts.size(); }
...
void Curve::moveControlPoints(const std::vector<int> &ivCtrlPts, const Point &ptOffset, const float fMinY,
                              const float fMaxY) {
    int iCtrlPtCount = m_ptvCtrlPts.size();
...
void Curve::drawEvaluatedCurveSegments() const {
    ...
    for (std::vector<Point>::const_iterator it = m_ptvEvaluatedCurvePts.begin(); it != m_ptvEvaluatedCurvePts.end();
         ++it) {
...
void Curve::drawControlPoints() const {
    ...
    glColor3d(1, 1, 1);
    glBegin(GL_POINTS);
    for (std::vector<Point>::const_iterator kit = m_ptvCtrlPts.begin(); kit != m_ptvCtrlPts.end(); ++kit) {
...
void Curve::sortControlPoints() const { std::sort(m_ptvCtrlPts.begin(), m_ptvCtrlPts.end(), PointSmallerXCompare()); }

void Curve::reevaluate() const {
    if (m_bDirty) {
        if (m_pceEvaluator) {
            if (m_ptvCtrlPts.size() <= 1) {
                m_pceEvaluator->handleEdgeCases(m_ptvCtrlPts, m_ptvEvaluatedCurvePts, m_fMaxX, default_value);
            } else {
                m_pceEvaluator->evaluateCurve(m_ptvCtrlPts, m_ptvEvaluatedCurvePts, m_fMaxX, m_bWrap, default_value);
            }

            std::sort(m_ptvEvaluatedCurvePts.begin(), m_ptvEvaluatedCurvePts.end(), PointSmallerXCompare());
...
void Curve::invalidate() const { m_bDirty = true; }

void Curve::set_default_value(float new_def) { default_value = new_def; }
